...
namespace Jplugin
{
    ...
    public class URebarTrimmer : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            {
                transaction.Start("Укоротить хвосты П-стержня");

                foreach (Element element in selectedURebars)
                {
                    operationResult = RebarLengthCorrector.TrimURebar(element, out string errorMessage);
