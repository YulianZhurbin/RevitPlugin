...
namespace Jplugin
{
    ...
    public class RebarStretcher : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            {
                transaction.Start("Увеличить длину стержня");

                foreach (Element element in selectedRebars)
                {
                    operationResult = RebarLengthCorrector.Stretch(element, out string errorMessage);
